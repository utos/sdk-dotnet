...
# docs/template-expressions.md. Each fixture trips exactly one rule. Run from this directory.
set -euo pipefail
...
cond() { # $1=name $2=condition $3=code
  bundle "https://api.example.com/hello" "[ { \"condition\": \"$2\", \"transition\": { \"name\": \"end\" } } ]" > "$1.json"
...
leaf() {
  bundle "https://api.example.com/hello" "[ { \"transition\": { \"name\": \"end\", \"input\": { \"value\": \"$2\" } } } ]" > "$1.json"
  expected "$1" "$3" "$W.onSuccess[0].transition.input.value"
}

rm -f expression-*.json
...
cond expression-new                     "new Date() !== null"                                 UTOS-E040
...
cond expression-binary-operator         "(1 | 2) === 3"                                       UTOS-E051
cond expression-assignment-operator     "let x = 1; x |= 2; x === 3"                          UTOS-E052
